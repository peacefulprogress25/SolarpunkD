...
contract EarthTreasury is Ownable {
     ...
     */
    function seedMint(uint256 amountStablec, uint256 amountEarth) external onlyOwner {
    ...
    function harvest(uint256 distributionPercent) external onlyOwner {
      ...
      uint256 totalSupplyEarth = EARTH.totalSupply() - EARTH.balanceOf(address(MINT_ALLOWANCE));
      uint256 impliedSupplyAtCurrentIVEarth = reserveStablec * intrinsicValueRatio.earth / intrinsicValueRatio.stablec;

      require(impliedSupplyAtCurrentIVEarth >= totalSupplyEarth, "Cannot run harvest when IV drops");

      uint256 newHarvestEarth = (impliedSupplyAtCurrentIVEarth - totalSupplyEarth) * distributionPercent / 100;
    ...
    function distributeHarvest() external onlyOwner {
      ...
      for (uint256 i = 0; i < pools.length; i++) {
        uint256 allocatedRewards = harvestedRewardsEarth * poolHarvestShare[pools[i]] / totalHarvestShares;
     ...
     */
    function mintAndAllocateEarth(address _contract, uint256 amountEarth) external onlyOwner {
      ...
      SafeERC20.safeIncreaseAllowance(EARTH, address(MINT_ALLOWANCE), amountEarth);
     ...
     */
    function unallocateAndBurnUnusedMintedEarth(address _contract) external onlyOwner {
