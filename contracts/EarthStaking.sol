...
contract EarthStaking is Ownable {
    ...
    EarthERC20Token immutable public EARTH; // The token being staked, for which EARTH rewards are generated
    Fruit immutable public FRUIT; // Token used to redeem staked EARTH
    ...
    function balance(uint256 amountFruit) public view returns(uint256) {
        return _overflowSafeMul1e18(
            ABDKMath64x64.divu(amountFruit, 1e18).mul(_accumulationFactorAt(currentEpoch()))
        );
    }
    ...
    function stakeFor(address _staker, uint256 _amountEarth) public returns(uint256 amountFruit) {
        require(_amountEarth > 0, "Cannot stake 0 tokens");

        _updateAccumulationFactor();

        // net past value/genesis value/Fruit Value for the earth you are putting in.
        amountFruit = _overflowSafeMul1e18(ABDKMath64x64.divu(_amountEarth, 1e18).div(accumulationFactor));

        SafeERC20.safeTransferFrom(EARTH, msg.sender, address(this), _amountEarth);
    ...
    /** Stake earth */
    function stake(uint256 _amountEarth) external returns(uint256 amountFruit) {
        return stakeFor(msg.sender, _amountEarth);
    }

    /** Unstake earth */
    function unstake(uint256 _amountFruit) external {      
        require(FRUIT.allowance(msg.sender, address(this)) >= _amountFruit, 'Insufficient fruit allowance. Cannot unstake');
